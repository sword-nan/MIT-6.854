...
func TestReElection3A(t *testing.T) {
	...
	DPrintf("A: Initial Leader is %d\n", leader1)
	// if the leader disconnects, a new one should be elected.
	DPrintf("disconnect sever %d\n", leader1)
	cfg.disconnect(leader1)
	l := cfg.checkOneLeader()
	// cfg.checkOneLeader()
	DPrintf("B: disconnect the initial leader %d, the new leader is %d\n", leader1, l)
	// if the old leader rejoins, that shouldn't
	// disturb the new leader. and the old leader
	// should switch to follower.
	DPrintf("connect sever %d\n", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	DPrintf("C: rejoin the initial leader %d, don't disturb the new leader %d\n", leader1, leader2)
	// if there's no quorum, no new leader should
	// be elected.
	DPrintf("disconnect sever %d and %d\n", leader2, (leader2+1)%servers)
...
func TestManyElections3A(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	// cfg.checkOneLeader()
	DPrintf("The Initial leader is %d\n", leader1)
	...
	for ii := 1; ii < iters; ii++ {
		...
		i2 := rand.Int() % servers
		i3 := rand.Int() % servers
		DPrintf("loop-%d: disconnect the sever %d %d %d\n", ii, i1, i2, i3)
		cfg.disconnect(i1)
		cfg.disconnect(i2)
		cfg.disconnect(i3)
		...
		// cfg.checkOneLeader()
		DPrintf("loop-%d: leader is %d\n", ii, leader)
		DPrintf("connect the sever %d %d %d\n", i1, i2, i3)
		cfg.connect(i1)
		cfg.connect(i2)
		cfg.connect(i3)
	}
...
func TestFailAgree3B(t *testing.T) {
	...
	DPrintf("disconnect the server %d", (leader+1)%servers)
	...
	DPrintf("connect the server %d", (leader+1)%servers)
...
loop:
	for try := 0; try < 5; try++ {
		...
		if !ok {
			DPrintf("leader moved on really quickly")
		...
		for ii := 0; ii < iters; ii++ { // 并发发送请求
			wg.Add(1)
			go func(i int) {
				defer wg.Done()
				i, term1, ok := cfg.rafts[leader].Start(100 + i)
				if term1 != term {
					DPrintf("leader 进行切换，放弃发送请求")
					return
				}
				if ok != true {
					return
...
func TestRejoin3B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	DPrintf("disconnect the sever %d\n", leader1)
